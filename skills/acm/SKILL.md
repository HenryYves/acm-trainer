...
After providing a solution, algorithm explanation, or code review, **automatically write the full content** to the `.md` file by appending. Do NOT overwrite — always append. Each new entry starts with a `---` separator line. Follow-up explanations (e.g., user asks "为什么XXX" about a previously written solution) should also be appended to the `.md` — the file is the complete reference.

**Terminal vs Markdown**: The `.md` file is the **primary reference** — write the complete, polished content there. In terminal, respond with a **concise summary** (1–3 sentences + key takeaway), then note "详细内容已写入 `<path>`"。不要在终端和 .md 里重复完整内容。原因：终端输出留在会话历史里，后续每轮都要重读；.md 文件不在历史里，只在追加时读一次。终端精简 = 未来每轮都省 token。

Exception: if the user is in a rapid back-and-forth Q&A flow, answer normally in terminal and batch-append to `.md` afterward. Quick one-line replies don't need to go to `.md`.
...
**Anchor links for appended follow-up**: When appending content that relates to an earlier section in the same file:
- In the **original section**, near the paragraph being supplemented, insert a forward link:
  ```
  > 📝 补充：[<title>](#<anchor>)
  ```
- At the **end of the new section**, insert a back link:
  ```
  > ⬆ 返回：[<title>](#<anchor>)
  ```
- Anchors: use GitHub/Typora auto-IDs (heading text → lowercase, spaces→hyphens, strip `?！。` etc.), or explicit `<a id="xxx"></a>` tags for precision.
- See `references/markdown-output.md` § 追加内容的双向链接 for full format.
